...
export default function Navigation() {
  ...
  useEffect(() => {
    const handleRouteChange = () => {
      if (popoverButtonRef.current && isPopoverOpen) {
  ...
  }, [router, isPopoverOpen]);

  useEffect(() => {
    // Close popover on user state change (login/logout)
    if (isLoaded && !isSignedIn && popoverButtonRef.current && isPopoverOpen) {
      popoverButtonRef.current.click();
    }
